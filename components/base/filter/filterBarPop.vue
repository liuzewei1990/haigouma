        ...
        <div class="filterbarpop-wrap" v-if="visible" :style="{'top': bgTop + 'px'}">
            ...
            <div class="filterbarpop">
                <div class="tab-bar" v-show="hasTabHeader">
                    <a href="javascript:;" :style="{'flex': column}" role="button" @click="clickTab(tab, index)" v-for="(tab, index) in menu.tabs" :key="index" :class="{'selected': selectIndexTab == index}">
                        <span :class="tab.icon"></span>{{tab.name}}</a>
                </div>
                <div class="main">
                    <div class="main-sidebar" :class="{'full-line': !items,'bg-style':items,'line-style':!items,}">
                        <div v-if="menu.type !== 'filter'" class="item" @click="clickSidebar(sidemenu, index)" v-for="(sidemenu, index) in sideMenus.detailList" :key="index" :class="{'selected': currentSelectIndex == index}">
                            <span :class="sidemenu.icon"></span>{{ sidemenu.name }}
                        </div>
                        <div v-if="menu.type == 'filter'" v-for="(sm, _index) in menu.tabs" :key="_index">
